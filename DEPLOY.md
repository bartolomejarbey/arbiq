...
4. **Environment Variables** — vložte všechny:
   ...
   | `RESEND_FROM` | `ARBIQ <noreply@arbey.cz>` |
   | `RESEND_BCC_ADMIN` | `bartolomej@arbey.cz` |
...
- Resend si ověří doménu a začne posílat e-maily z `noreply@arbey.cz`.
...
- **Vlastní `noreply@arbey.cz` e-mailovou schránku** přes Resend forwarding (Inbound) nebo přidat MX záznam k Vašemu mail providerovi.
