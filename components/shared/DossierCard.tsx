...
export default function DossierCard({ label, title, children, rotation = "-rotate-3", className = "" }: DossierCardProps) {
  return (
    <div className={`relative bg-parchment text-espresso-text p-6 ${rotation} ${className}`}>
      <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-12 h-3 bg-tobacco"></div>
