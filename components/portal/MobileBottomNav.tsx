...
export default function MobileBottomNav() {
  ...
  return (
    <nav aria-label="Mobilní navigace" className="md:hidden fixed bottom-0 left-0 right-0 bg-coffee border-t border-tobacco z-30 pb-[env(safe-area-inset-bottom)]">
      <ul className="flex">
        {items.map((item) => {
          ...
          return (
            <li key={item.href} className="flex-1 min-w-0">
              <Link
                ...
                className={`flex flex-col items-center gap-1 py-3 min-h-[44px] text-[9px] font-mono uppercase tracking-widest transition-colors ${
