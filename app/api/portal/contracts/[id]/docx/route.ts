...
export async function GET(
  ...
  try {
    const buf = await downloadDocument(r.docx_url);
    return new NextResponse(new Uint8Array(buf), {
      status: 200,
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        'Content-Disposition': `attachment; filename="Smlouva-${r.contract_number}.docx"`,
        'Cache-Control': 'private, no-store',
      },
    });
  } catch (err) {
    console.error('[CONTRACT DOCX] download', err);
